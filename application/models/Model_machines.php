...
class Model_machines extends CI_Model
{
	...
	{
		if($id) {
			$this->db->select('machines.*,machine_heads.name as machine_heads_name');
			$this->db->from('machines');
			$this->db->join('machine_heads', 'machine_heads.id = machines.heads_id', 'left');
			$this->db->join('machine_service_type', 'machine_service_type.id = machines.service_type_id', 'left');
			$this->db->where('machines.id',$id);
			$this->db->where('machines.data_status','A');
			return $this->db->get()->row_array();
       		
    	}

		$this->db->select('machines.*,machine_heads.name as heads_name');
		$this->db->from('machines');
		$this->db->join('machine_heads', 'machine_heads.id = machines.heads_id', 'left');
		$this->db->join('machine_service_type', 'machine_service_type.id = machines.service_type_id', 'left');
		$this->db->where('machines.data_status','A');
	...
	{
		$sql = "SELECT fa.*,fah.name as fixed_asset_heads_name,fst.name as fixed_asset_service_type_name FROM `machines` fa
		LEFT JOIN  `machine_heads` fah ON fah.id = fa.heads_id 
		LEFT JOIN  `machine_service_type` fst ON fst.id = fa.service_type_id 
		WHERE fa.data_status = 'A' ";

		if(!empty($data['filter_fixed_asset_heads_id'])){
			$sql.=' AND fa.heads_id = "'.$data['filter_fixed_asset_heads_id'].'"  ';
		}

		if(!empty($data['filter_fixed_asset_service_type_id'])){
			$sql.=' AND fa.service_type_id = "'.$data['filter_fixed_asset_service_type_id'].'"  ';
		}
	...
	{
		$sql = "SELECT fa.id FROM `machines` fa
		LEFT JOIN  `machine_heads` fah ON fah.id = fa.heads_id 
		LEFT JOIN  `machine_service_type` fst ON fst.id = fa.service_type_id 
		WHERE fa.data_status = 'A' ";
