...
class Machines extends Admin_Controller 
{
	...
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Machines';

		$this->load->model('model_machines');
		$this->load->model('model_machine_heads');
		$this->load->model('model_machine_service_type');
	...
	*/
	...
	{
		if(!in_array('viewMachines', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
		$data = $this->model_machines->getData();
		$this->data['fixed_assest_heads'] = $this->model_machine_heads->getData();
		$this->data['fixed_assest_service_types'] = $this->model_machine_service_type->getData();
		$this->getList();
	}	
	...
	protected function getList()
	{
		...
		$this->data['fixed_assests']=$this->model_machines->getDataList($filter);
		$total_item=$this->model_machines->getDataListTotal($filter);
	...
	{
		if($id) {
			$data = $this->model_machines->getData($id);
	...
	public function fetchData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '';
			if(in_array('updateMachines', $this->permission)) {
				$buttons .= '<button type="button" class="btn btn-default" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i></button>';
			}

			if(in_array('deleteMachines', $this->permission)) {
	...
	{
		if(!in_array('createMachines', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$data = array(
				'heads_id' 					=> $this->input->post('fixed_asset_heads_id'),
				'service_type_id' 			=> $this->input->post('fixed_asset_service_type_id'),
	...
	public function update($id)
	{
		...
		if($id) {
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	$data = array(
	        		'heads_id' 					=> $this->input->post('edit_fixed_asset_heads_id'),
					'service_type_id' 			=> $this->input->post('edit_fixed_asset_service_type_id'),
	        	...
	        	$update = $this->model_machines->update($data, $id);
	...
	public function export_report()
	{
		...
		$results = $this->model_machines->getExportDataList($filter);
		$spreadsheet = new Spreadsheet(); // instantiate Spreadsheet
        $sheet = $spreadsheet->getActiveSheet();

        // manually set table data value
        $sheet->setCellValue('A1', 'Machine Heads'); 
	...
	public function remove()
	{
		...
		$response = array();
		if($id) {
			$delete = $this->model_machines->remove($id);
			if($delete == true) {
				$response['success'] = true;
