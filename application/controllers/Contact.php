...
class Contact extends Admin_Controller 
{
	...
	*/
	public function index()
	{
		...
		$this->render_template('contact/index', $this->data);	
	}	
	...
	*/
	...
	{
		if($id) {
			$data = $this->model_contact->getData($id);
	...
	*/
	public function fetchData()
	{
		...
		$data = $this->model_contact->getData();
		...
		foreach ($data as $key => $value) {
			...
			if(in_array('updateContact', $this->permission)) {
	...
	*/
	...
	{
		if(!in_array('createContact', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
        ...
        $this->form_validation->set_rules('email', 'Email', 'trim|required|valid_email|is_unique[contacts.email]');
		$this->form_validation->set_rules('phone', 'Phone', 'trim|required|is_unique[contacts.phone]');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$create = $this->model_contact->create($data);
        	if($create == true) {
	...
	*/
	public function update($id)
	{
		...
		if($id) {
            ...
            $this->form_validation->set_rules('edit_email', 'Email', 'trim|required|valid_email|callback_email_check');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	...
	        	$update = $this->model_contact->update($data, $id);
	        	if($update == true) {
	        		$response['success'] = true;
    ...
    public function email_check($email) {
        ...
        if ($query->num_rows() > 0) {
            $this->form_validation->set_message('email_check', 'The {field} is already in use.');
            return FALSE;
        } else {
            return TRUE;
        }
	...
	*/
	public function remove()
	{
		...
		$response = array();
		if($id) {
			$delete = $this->model_contact->remove($id);
			if($delete == true) {
				$response['success'] = true;
