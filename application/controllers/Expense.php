...
class Expense extends Admin_Controller 
{
	...
	public function __construct()
	{
		...
		$this->load->model('model_expense_heads');
	...
	*/
	public function index()
	{
		if(!in_array('viewExpense', $this->permission)) {
		 ...
		 $this->data['expense_heads'] = $this->model_expense_heads->getData();
		//$this->render_template('expense/index', $this->data);	
		$this->getList();
	}	
	...
	protected function getList()
	{
		...
		if( $this->input->get('filter_from_date') || $this->input->get('filter_to_date') || $this->input->get('filter_expense_heads_id') ){
			$is_report_download = true;
		}


		$filter=array(
			"filter_expense_heads_id"	  =>$this->input->get('filter_expense_heads_id'),
		...
		$this->data['expenses']=$this->model_expense->getDataList($filter);
		
		$total_item=$this->model_expense->getDataListTotal($filter);
	   ...
	   $this->data['filter_expense_heads_id'] = $this->input->get('filter_expense_heads_id');
	...
	*/
	...
	{
		if($id) {
			$data = $this->model_expense->getData($id);
	...
	*/
	public function fetchData()
	{
		...
		$data = $this->model_expense->getData();
	...
	*/
	public function create()
	{
		...
		$this->form_validation->set_rules('expense_heads_id', 'Expense Heads', 'trim|required');
        $this->form_validation->set_rules('amount', 'Expense Amount', 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$data = array(
        		'expense_heads_id' 			=> $this->input->post('expense_heads_id'),
        	...
        	$create = $this->model_expense->create($data);
        	if($create == true) {
	...
	*/
	public function update($id)
	{
		...
		if($id) {
			...
			$this->form_validation->set_rules('edit_expense_heads_id', 'Expense Heads', 'trim|required');
            $this->form_validation->set_rules('edit_amount', 'Expense Amount', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
	        	$data = array(
	        		'expense_heads_id' 			=> $this->input->post('edit_expense_heads_id'),
	        	...
	        	$update = $this->model_expense->update($data, $id);
				...
				if($_FILES['edit_reciept_file']['size'] > 0) {
					...
					$this->model_expense->update($upload_file,$id);
            	}
	...
	public function upload_file($field_expense_heads_id)
    {
    	// assets/images/logo
        $config['upload_path'] = 'uploads/reciept';
        $config['file_expense_heads_id'] =  uniqid();
        ...
        if (!$this->upload->do_upload($field_expense_heads_id))
        {
        ...
        else
        {
            ...
            $type = explode('.', $_FILES[$field_expense_heads_id]['expense_heads_id']);
            $type = $type[count($type) - 1];
            
            $path = $config['upload_path'].'/'.$config['file_expense_heads_id'].'.'.$type;
	...
	public function export_report()
	{
		...
		$results = $this->model_expense->getExportDataList($filter);
        ...
        $sheet->setCellValue('A1', 'Expense Heads'); 
		...
		if($results){
			...
			foreach($results as $row){
			  ...
			  $total = $total + $row['amount'];
				$sheet->setCellValue('A'.$inc, $row['expense_heads_name']); 
				$sheet->setCellValue('B'.$inc, $row['amount']);
				$sheet->setCellValue('C'.$inc, date("d/m/Y",strtotime($row['from_date'])));
				$sheet->setCellValue('D'.$inc, date("d/m/Y",strtotime($row['to_date'])));
				$sheet->setCellValue('E'.$inc, $row['details']);
        ...
        $filename = 'EXpenses - '.date("d-m-Y"); // set filename for excel file to be exported
	...
	*/
	public function remove()
	{
		...
		$response = array();
		if($id) {
			$delete = $this->model_expense->remove($id);
			if($delete == true) {
				$response['success'] = true;
