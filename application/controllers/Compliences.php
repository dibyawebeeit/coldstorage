...
class Compliences extends Admin_Controller 
{
	...
	public function __construct()
	{
		...
		$this->data['page_title'] = 'Compliences';

		$this->load->model('model_compliences');
		$this->load->model('model_compliences_heads');
		$this->load->model('model_compliences_service_type');
	...
	*/
	...
	{
		if(!in_array('viewCompliences', $this->permission)) {
			redirect('dashboard', 'refresh');
		}
		$data = $this->model_compliences->getData();
		$this->data['complience_heads'] = $this->model_compliences_heads->getData();
		$this->data['complience_service_types'] = $this->model_compliences_service_type->getData();
		$this->getList();
	}	
	...
	protected function getList()
	{
		...
		if( $this->input->get('filter_from_date') || $this->input->get('filter_to_date') || $this->input->get('filter_compliences_heads_id') ){
			$is_report_download = true;
		}


		$filter=array(
			"filter_compliences_heads_id"	  		  =>$this->input->get('filter_compliences_heads_id'),
			"filter_compliences_service_type_id"	  =>$this->input->get('filter_compliences_service_type_id'),
		...
		$this->data['compliences']=$this->model_compliences->getDataList($filter);
		$total_item=$this->model_compliences->getDataListTotal($filter);
	    ...
	    $this->data['filter_compliences_heads_id'] = $this->input->get('filter_compliences_heads_id');
		$this->data['filter_compliences_service_type_id'] = $this->input->get('filter_compliences_service_type_id');
		...
		$this->render_template('compliences/index', $this->data);
	}
	...
	*/
	...
	{
		if($id) {
			$data = $this->model_compliences->getData($id);
	...
	*/
	public function fetchData()
	{
		...
		foreach ($data as $key => $value) {
			...
			// button
			$buttons = '';
			if(in_array('updateCompliences', $this->permission)) {
				$buttons .= '<button type="button" class="btn btn-default" onclick="editFunc('.$value['id'].')" data-toggle="modal" data-target="#editModal"><i class="fa fa-pencil"></i></button>';
			}

			if(in_array('deleteCompliences', $this->permission)) {
			...
			$result['data'][$key] = array(
				$value['compliences_heads_name'],
				$value['compliences_service_type_name'],
				$value['start_date'],
				$value['details'],
				$buttons
			);
	...
	*/
	...
	{
		if(!in_array('createCompliences', $this->permission)) {
			redirect('dashboard', 'refresh');
		}

		$response = array();

		$this->form_validation->set_rules('compliences_heads_id', 'Compliences Heads', 'trim|required');
		$this->form_validation->set_rules('compliences_service_type_id', 'Compliences Heads', 'trim|required');
		$this->form_validation->set_rules('start_date', 'Date', 'trim|required');
        $this->form_validation->set_rules('service_duration', 'Service Duration', 'trim|required');
        ...
        if ($this->form_validation->run() == TRUE) {
        	...
        	$data = array(
				'compliences_heads_id' 					=> $this->input->post('compliences_heads_id'),
				'compliences_service_type_id' 			=> $this->input->post('compliences_service_type_id'),
        	...
        	$create = $this->model_compliences->create($data);
        	if($create == true) {
	...
	*/
	public function update($id)
	{
		...
		if($id) {
			...
			$this->form_validation->set_rules('edit_compliences_heads_id', 'Compliences Heads', 'trim|required');
			$this->form_validation->set_rules('edit_compliences_service_type_id', 'Compliences Service Type', 'trim|required');
			$this->form_validation->set_rules('edit_start_date', 'Date', 'trim|required');
            $this->form_validation->set_rules('edit_service_duration', 'Service Duration', 'trim|required');
	        ...
	        if ($this->form_validation->run() == TRUE) {
				...
				$date = new DateTime($this->input->post('edit_start_date'));
				$date->modify('+'.$this->input->post('edit_service_duration').' months');
				$service_date = $date->format('Y-m-d');

	        	$data = array(
	        		'compliences_heads_id' 					=> $this->input->post('edit_compliences_heads_id'),
					'compliences_service_type_id' 			=> $this->input->post('edit_compliences_service_type_id'),
	        	...
	        	$update = $this->model_compliences->update($data, $id);
				...
				/*if($_FILES['edit_reciept_file']['size'] > 0) {
					...
					$this->model_compliences->update($upload_file,$id);
            	}*/
	...
	public function upload_file($field_compliences_heads_id)
    {
    	// assets/images/logo
        $config['upload_path'] = 'uploads/reciept';
        $config['file_compliences_heads_id'] =  uniqid();
        ...
        if (!$this->upload->do_upload($field_compliences_heads_id))
        {
        ...
        else
        {
            ...
            $type = explode('.', $_FILES[$field_compliences_heads_id]['compliences_heads_id']);
            $type = $type[count($type) - 1];
            
            $path = $config['upload_path'].'/'.$config['file_compliences_heads_id'].'.'.$type;
	...
	public function export_report()
	{
		...
		$results = $this->model_compliences->getExportDataList($filter);
		$spreadsheet = new Spreadsheet(); // instantiate Spreadsheet
        $sheet = $spreadsheet->getActiveSheet();

        // manually set table data value
        $sheet->setCellValue('A1', 'Compliences Heads'); 
		...
		if($results){
			...
			foreach($results as $row){
				...
				$sheet->setCellValue('A'.$inc, $row['compliences_heads_name']); 
				$sheet->setCellValue('B'.$inc, $row['compliences_service_type_name']);
        ...
        $filename = 'Compliences - '.date("d-m-Y"); // set filename for excel file to be exported
	...
	*/
	public function remove()
	{
		...
		$response = array();
		if($id) {
			$delete = $this->model_compliences->remove($id);
			if($delete == true) {
				$response['success'] = true;
